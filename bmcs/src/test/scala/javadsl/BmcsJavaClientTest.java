...
public class BmcsJavaClientTest {
    ...
    private final String keyPath = "/ssd/scala_github/alpakka/oci_api_key.pem";
    ...
    private final BmcsSettings settings = BmcsSettings.create(ConfigFactory.load());

    private final String bucket = "CEGBU_Prime";

    private final BmcsClient client = new BmcsClient(settings, cred, system, materializer);
    ...
    public void testSomething() throws ExecutionException, InterruptedException {
        ...
        try (InputStream in = new FileInputStream(new File("/ssd/scala_github/alpakka/ehs.pdf"))) {
            ...
            System.out.println(multipartUploadResult.etag());
            Assert.assertNotNull(multipartUploadResult);
